...
class _LessonsPageState extends State<LessonsPage> with TickerProviderStateMixin {
  ...
  Future<void> _generateAndPlayVoice(String text) async {
    try {
      ...
      // Update the backend URL if it has changed or expired
      final response = await http.post(
        Uri.parse('https://268b-196-184-222-196.ngrok-free.app/generate-voice'), // Verify this URL is active
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({
          "text": text,
          "speaker_wav": "sounds/SpongBob.wav", // Ensure this file exists on the server
        }),
      );

      if (response.statusCode == 200) {
        // Update the audio URL if the local server is different or unavailable
        final String audioUrl = 'http://172.20.10.9:8001/outputlive.wav'; // Verify this IP and port are correct
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Error playing audio: $e. Please check your network or server."),
...
Future<void> _checkAndOpenActivity(String description) async {
    ...
    final activityUrl = "https://8fd8-102-154-202-95.ngrok-free.app/KiddoAI/Activity/problem";
    final activityPageUrl = "http://172.20.10.4:8080/";
